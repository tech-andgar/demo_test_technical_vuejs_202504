...
describe('Configuración de Tailwind', () => {
  it('tiene la configuración básica correcta', () => {
    ...
    expect(tailwindConfig.content.some(path => path.includes('src') && path.includes('vue'))).toBe(true)
    expect(tailwindConfig.content.some(path => path.includes('tailwind.purge.safelist.txt'))).toBe(true)
